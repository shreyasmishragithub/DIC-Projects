...
plt.title('India Covid19 Daywise Cure Prediction')
...
df = pd.read_csv('covidIndiamade.csv', usecols = ['Cured'])
...
df_old = pd.read_csv('covidIndia1.csv', usecols = ['Date', 'Cured'])
...
plt.title('India Covid19 Daywise Cured Prediction')
plt.xticks(rotation=60)
plt.tight_layout()
plt.xlabel('Days')
plt.ylabel('Cases')
...
df_54 = copy[:54].copy(deep = True)
df_6 = copy[54:].copy(deep = True)
#df_54.columns = ['Confirmed']
#df_6.columns = ['Predictions']
df_54.plot()
df_6.plot()

df_all = pd.merge(df_54, df_6, how = 'outer', left_index=True, right_index=True)
df_all.plot()
'''
